...
def multipolygon(coord):
    ...
    for poly in coord:
        for lst in poly:
            ...
            nb_pt = len(pts_temp)
            poly = c4d.CPolygon(id_pt,id_pt+1,id_pt+2)
            polys.append(poly)
            id_pt+=nb_pt
    ...
    pcnt = len(pts)
    polycnt = len(polys)
    res = c4d.PolygonObject(pcnt,polycnt)
    #res = c4d.SplineObject(pcnt,c4d.SPLINETYPE_LINEAR)
    #res[c4d.SPLINEOBJECT_CLOSED] = True
    res.SetAllPoints(pts)
    res.SetAbsPos(centre)
    ...
    #on optimize les points (toutes les faces sont séparées)
    settings = c4d.BaseContainer()  # Settings
    settings[c4d.MDATA_OPTIMIZE_TOLERANCE] = 0.01  # Length of the extrusion
    settings[c4d.MDATA_OPTIMIZE_POINTS] = True
    settings[c4d.MDATA_OPTIMIZE_POLYGONS] = False
    settings[c4d.MDATA_OPTIMIZE_UNUSEDPOINTS] = False


    r = c4d.utils.SendModelingCommand(command=c4d.MCOMMAND_OPTIMIZE,
                                        list=[res],
