# BlenderGIS code adapted for Cinema4D (source: https://github.com/domlysz/BlenderGIS)

# Warning, the y and z values are reversed, in Cinema4D the y represents the altitude!
...
class BBOX(dict):
	...
	def dimensions(self):
		...
		if self.hasZ:
			...
			return c4d.Vector(dx,dz,dy)
		else:
			return c4d.Vector(dx,0,dy)
	...
	def ul(self):
		'''upper left corner'''
		return c4d.Vector(self.xmin, 0, self.ymax)
	@property
	def ur(self):
		'''upper right corner'''
		return c4d.Vector(self.xmax, 0, self.ymax)
	@property
	def bl(self):
		'''bottom left corner'''
		return c4d.Vector(self.xmin, 0, self.ymin)
	@property
	def br(self):
		'''bottom right corner'''
		return c4d.Vector(self.xmax, 0, self.ymin)
