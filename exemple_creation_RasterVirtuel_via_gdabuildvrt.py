...
# Main function
def main():
    #Example création de raster virtuel
    path_tifs = '/Users/olivierdonze/Documents/TEMP/test_dwnld_swisstopo/test/swissimage-dop10'
    fn_vrt = (createVRTfromDir(path_tifs, path_to_gdalbuildvrt = None))
    
    if fn_vrt:
        #Example extraction via gdal_translate
        raster_srce = fn_vrt
        raster_dst = fn_vrt.replace('.vrt','.jpg')
        xmin,ymin,xmax,ymax = 2499118.296588665,1119370.5071857648,2500886.9513362795,1120645.2426812807
        print(extractFromBbox(raster_srce, raster_dst,xmin,ymin,xmax,ymax,path_to_gdal_translate = None))
        return
    ...
    if path_to_QGIS_bin:
        gdalbuildvrt = gdalBIN_OK(path_to_QGIS_bin, exe = 'gdalbuildvrt')
        fn_vrt = None

        if gdalbuildvrt :
            path_tifs = '/Users/olivierdonze/Documents/TEMP/test_dwnld_swisstopo/test/swissalti3d'
            createVRTfromDir(path_tifs, gdalbuildvrt)
            

        xmin,ymin,xmax,ymax = 2499008.774143775,1116615.2606724824,2500479.2359311297,1117688.864875182
        gdal_translate = gdalBIN_OK(path_to_QGIS_bin, exe = 'gdal_translate',win = win)

        if gdal_translate and fn_vrt:
            fn_ascii = fn_vrt.replace('.vrt','.asc')
            req = f'{gdal_translate} -of AAIGrid -projwin {xmin} {ymax} {xmax} {ymin} {fn_vrt} {fn_ascii}'
            output = subprocess.check_output(req,shell=True)
